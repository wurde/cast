...
class InitElectronSimple {
  ...
  run() {
    ...
    let tasks = new Listr([
      ...
      { title: 'add_desktop_dir()', task: this.add_desktop_dir },
  ...
  add_readme(ctx) {
    ...
    text = text.replace('__DESCRIPTION__', 'A simple Electron desktop app.')
  ...
  add_package_json(ctx) {
    let text = fs.readFileSync(path.join(__dirname, 'files', 'electron-simple', 'package.json'), 'utf8')
    text = text.replace(/__TITLE__/g, ctx.cwd_basename)
    if (!fs.existsSync('package.json')) { fs.writeFileSync('package.json', text) }
  }

  add_webpack_config() {
    if (!fs.existsSync('webpack.config.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'webpack.config.js'), 'webpack.config.js') }
  }
  ...
  add_client_dir() {
    ...
    if (!fs.existsSync('client/dist/.keep')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'dist', '.keep'), 'client/dist/.keep') }

    if (!fs.existsSync('client/public')) { fs.mkdirSync('client/public') }
    if (!fs.existsSync('client/public/index.html')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'public', 'index.html'), 'client/public/index.html') }

    if (!fs.existsSync('client/src')) { fs.mkdirSync('client/src') }
    if (!fs.existsSync('client/src/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'src', 'index.js'), 'client/src/index.js') }
    if (!fs.existsSync('client/src/App.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'src', 'App.js'), 'client/src/App.js') }
    if (!fs.existsSync('client/src/App.scss')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'src', 'App.scss'), 'client/src/App.scss') }

    if (!fs.existsSync('client/src/components')) { fs.mkdirSync('client/src/components') }
    if (!fs.existsSync('client/src/components/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'src', 'components', 'index.js'), 'client/src/components/index.js') }
    ...
    if (!fs.existsSync('client/src/components/Visualization/Visualization.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'src', 'components', 'Visualization', 'Visualization.js'), 'client/src/components/Visualization/Visualization.js') }
    if (!fs.existsSync('client/src/components/Visualization/styles')) { fs.mkdirSync('client/src/components/Visualization/styles') }
    if (!fs.existsSync('client/src/components/Visualization/styles/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'src', 'components', 'Visualization', 'styles', 'index.js'), 'client/src/components/Visualization/styles/index.js') }
    if (!fs.existsSync('client/src/components/Visualization/styles/VisualizationStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'client', 'src', 'components', 'Visualization', 'styles', 'VisualizationStyle.js'), 'client/src/components/Visualization/styles/VisualizationStyle.js') }
  }
  ...
  add_server_dir() {
    ...
    if (!fs.existsSync('server/app.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'server', 'app.js'), 'server/app.js') }
  }

  add_desktop_dir() {
    if (!fs.existsSync('desktop')) { fs.mkdirSync('desktop') }
    if (!fs.existsSync('desktop/app.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'desktop', 'app.js'), 'desktop/app.js') }
  }
  ...
  add_test_dir() {
    ...
    if (!fs.existsSync('test/features/title_test')) { fs.copyFileSync(path.join(__dirname, 'files', 'electron-simple', 'test', 'features', 'title_test.js'), 'test/features/title_test.js') }
  ...
  npm_install_deps() {
    if (!fs.existsSync('package-lock.json')) {
      child_process.execSync('npm install electron --save --loglevel=error')
...
module.exports = InitElectronSimple
