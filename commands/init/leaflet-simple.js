...
class InitLeafletSimple {
  ...
  add_readme(ctx) {
    ...
    text = text.replace('__DESCRIPTION__', 'A simple React app for building Leaflet maps.')
  ...
  add_package_json(ctx) {
    let text = fs.readFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'package.json'), 'utf8')
    text = text.replace(/__TITLE__/g, ctx.cwd_basename)
    if (!fs.existsSync('package.json')) { fs.writeFileSync('package.json', text) }
  }

  add_webpack_config() {
    if (!fs.existsSync('webpack.config.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'webpack.config.js'), 'webpack.config.js') }
  }
  ...
  add_client_dir() {
    ...
    if (!fs.existsSync('client/dist/.keep')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'dist', '.keep'), 'client/dist/.keep') }

    if (!fs.existsSync('client/public')) { fs.mkdirSync('client/public') }
    if (!fs.existsSync('client/public/index.html')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'public', 'index.html'), 'client/public/index.html') }

    if (!fs.existsSync('client/src')) { fs.mkdirSync('client/src') }
    if (!fs.existsSync('client/src/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'src', 'index.js'), 'client/src/index.js') }
    if (!fs.existsSync('client/src/App.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'src', 'App.js'), 'client/src/App.js') }
    if (!fs.existsSync('client/src/App.scss')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'src', 'App.scss'), 'client/src/App.scss') }

    if (!fs.existsSync('client/src/components')) { fs.mkdirSync('client/src/components') }
    if (!fs.existsSync('client/src/components/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'src', 'components', 'index.js'), 'client/src/components/index.js') }
    ...
    if (!fs.existsSync('client/src/components/Map')) { fs.mkdirSync('client/src/components/Map') }
    if (!fs.existsSync('client/src/components/Map/Map.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'src', 'components', 'Map', 'Map.js'), 'client/src/components/Map/Map.js') }
    if (!fs.existsSync('client/src/components/Map/styles')) { fs.mkdirSync('client/src/components/Map/styles') }
    if (!fs.existsSync('client/src/components/Map/styles/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'src', 'components', 'Map', 'styles', 'index.js'), 'client/src/components/Map/styles/index.js') }
    if (!fs.existsSync('client/src/components/Map/styles/MapStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'client', 'src', 'components', 'Map', 'styles', 'MapStyle.js'), 'client/src/components/Map/styles/MapStyle.js') }
  }
  ...
  add_server_dir() {
    ...
    if (!fs.existsSync('server/app.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'server', 'app.js'), 'server/app.js') }
  }
  ...
  add_test_dir() {
    ...
    if (!fs.existsSync('test/features/title_test')) { fs.copyFileSync(path.join(__dirname, 'files', 'leaflet-simple', 'test', 'features', 'title_test.js'), 'test/features/title_test.js') }
    ...
    if (!fs.existsSync('package-lock.json')) {
      ...
      child_process.execSync(`\
        npm install react react-dom react-router-dom \
          ...
          html-webpack-plugin \
          ava puppeteer \
          leaflet --save-dev --loglevel=error
      `.trim().replace(/\s\s+/g, ' '))
    }
  }
...
module.exports = InitLeafletSimple
