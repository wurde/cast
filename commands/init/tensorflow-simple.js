...
class InitTensorflowSimple {
  ...
  add_readme(ctx) {
    ...
    text = text.replace('__DESCRIPTION__', 'A simple React app for TensorFlow models.')
  ...
  add_package_json(ctx) {
    let text = fs.readFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'package.json'), 'utf8')
    text = text.replace(/__TITLE__/g, ctx.cwd_basename)
    if (!fs.existsSync('package.json')) { fs.writeFileSync('package.json', text) }
  }

  add_webpack_config() {
    if (!fs.existsSync('webpack.config.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'webpack.config.js'), 'webpack.config.js') }
  }
  ...
  add_client_dir() {
    ...
    if (!fs.existsSync('client/dist/.keep')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'dist', '.keep'), 'client/dist/.keep') }

    if (!fs.existsSync('client/public')) { fs.mkdirSync('client/public') }
    if (!fs.existsSync('client/public/index.html')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'public', 'index.html'), 'client/public/index.html') }

    if (!fs.existsSync('client/src')) { fs.mkdirSync('client/src') }
    if (!fs.existsSync('client/src/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'src', 'index.js'), 'client/src/index.js') }
    if (!fs.existsSync('client/src/App.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'src', 'App.js'), 'client/src/App.js') }
    if (!fs.existsSync('client/src/App.scss')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'src', 'App.scss'), 'client/src/App.scss') }

    if (!fs.existsSync('client/src/components')) { fs.mkdirSync('client/src/components') }
    if (!fs.existsSync('client/src/components/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'src', 'components', 'index.js'), 'client/src/components/index.js') }
    ...
    if (!fs.existsSync('client/src/components/Model')) { fs.mkdirSync('client/src/components/Model') }
    if (!fs.existsSync('client/src/components/Model/Model.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'src', 'components', 'Model', 'Model.js'), 'client/src/components/Model/Model.js') }
    if (!fs.existsSync('client/src/components/Model/styles')) { fs.mkdirSync('client/src/components/Model/styles') }
    if (!fs.existsSync('client/src/components/Model/styles/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'src', 'components', 'Model', 'styles', 'index.js'), 'client/src/components/Model/styles/index.js') }
    if (!fs.existsSync('client/src/components/Model/styles/ModelStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'client', 'src', 'components', 'Model', 'styles', 'ModelStyle.js'), 'client/src/components/Model/styles/ModelStyle.js') }
  }
  ...
  add_server_dir() {
    ...
    if (!fs.existsSync('server/app.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'server', 'app.js'), 'server/app.js') }
  }
  ...
  add_test_dir() {
    ...
    if (!fs.existsSync('test/features/title_test')) { fs.copyFileSync(path.join(__dirname, 'files', 'tensorflow-simple', 'test', 'features', 'title_test.js'), 'test/features/title_test.js') }
    ...
    if (!fs.existsSync('package-lock.json')) {
      ...
      child_process.execSync(`\
        npm install react react-dom react-router-dom \
          ...
          html-webpack-plugin \
          ava puppeteer \
          @tensorflow/tfjs @tensorflow/tfjs-vis --save-dev --loglevel=error
      `.trim().replace(/\s\s+/g, ' '))
    }
  }
...
module.exports = InitTensorflowSimple
