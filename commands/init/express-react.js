...
class InitExpressReact {
  ...
  add_readme(ctx) {
    ...
    text = text.replace('__DESCRIPTION__', 'A simple React app.')
  ...
  add_package_json(ctx) {
    let text = fs.readFileSync(path.join(__dirname, 'files', 'express-react', 'package.json'), 'utf8')
    text = text.replace(/__TITLE__/g, ctx.cwd_basename)
    if (!fs.existsSync('package.json')) { fs.writeFileSync('package.json', text) }
  }

  add_webpack_config() {
    if (!fs.existsSync('webpack.config.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'webpack.config.js'), 'webpack.config.js') }
  }
  ...
  add_client_dir() {
    ...
    if (!fs.existsSync('client/dist/.keep')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'dist', '.keep'), 'client/dist/.keep') }

    if (!fs.existsSync('client/public')) { fs.mkdirSync('client/public') }
    if (!fs.existsSync('client/public/index.html')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'public', 'index.html'), 'client/public/index.html') }

    if (!fs.existsSync('client/src')) { fs.mkdirSync('client/src') }
    if (!fs.existsSync('client/src/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'index.js'), 'client/src/index.js') }
    if (!fs.existsSync('client/src/App.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'App.js'), 'client/src/App.js') }
    if (!fs.existsSync('client/src/App.scss')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'App.scss'), 'client/src/App.scss') }

    if (!fs.existsSync('client/src/components')) { fs.mkdirSync('client/src/components') }
    if (!fs.existsSync('client/src/components/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'index.js'), 'client/src/components/index.js') }
    ...
    if (!fs.existsSync('client/src/components/Error/styles/ErrorBoundaryStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'all-purpose', 'client', 'src', 'components', 'Error', 'styles', 'ErrorBoundaryStyle.js'), 'client/src/components/Error/styles/ErrorBoundaryStyle.js') }
    if (!fs.existsSync('client/src/components/Error/styles/Error404Style.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'all-purpose', 'client', 'src', 'components', 'Error', 'styles', 'Error404Style.js'), 'client/src/components/Error/styles/Error404Style.js') }
    if (!fs.existsSync('client/src/components/Error/styles/Error500Style.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'all-purpose', 'client', 'src', 'components', 'Error', 'styles', 'Error500Style.js'), 'client/src/components/Error/styles/Error500Style.js') }

    if (!fs.existsSync('client/src/components/Content')) { fs.mkdirSync('client/src/components/Content') }
    if (!fs.existsSync('client/src/components/Content/Content.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'Content.js'), 'client/src/components/Content/Content.js') }
    if (!fs.existsSync('client/src/components/Content/ContentForm.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'ContentForm.js'), 'client/src/components/Content/ContentForm.js') }
    if (!fs.existsSync('client/src/components/Content/LazyContent.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'LazyContent.js'), 'client/src/components/Content/LazyContent.js') }
    if (!fs.existsSync('client/src/components/Content/LoadingContent.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'LoadingContent.js'), 'client/src/components/Content/LoadingContent.js') }
    if (!fs.existsSync('client/src/components/Content/styles')) { fs.mkdirSync('client/src/components/Content/styles') }
    if (!fs.existsSync('client/src/components/Content/styles/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'styles', 'index.js'), 'client/src/components/Content/styles/index.js') }
    if (!fs.existsSync('client/src/components/Content/styles/ContentStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'styles', 'ContentStyle.js'), 'client/src/components/Content/styles/ContentStyle.js') }
    if (!fs.existsSync('client/src/components/Content/styles/ContentFormStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'styles', 'ContentFormStyle.js'), 'client/src/components/Content/styles/ContentFormStyle.js') }
    if (!fs.existsSync('client/src/components/Content/styles/LazyContentStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'styles', 'LazyContentStyle.js'), 'client/src/components/Content/styles/LazyContentStyle.js') }
    if (!fs.existsSync('client/src/components/Content/styles/LoadingContentStyle.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'components', 'Content', 'styles', 'LoadingContentStyle.js'), 'client/src/components/Content/styles/LoadingContentStyle.js') }

    if (!fs.existsSync('client/src/routes')) { fs.mkdirSync('client/src/routes') }
    if (!fs.existsSync('client/src/routes/index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'routes', 'index.js'), 'client/src/routes/index.js') }
    if (!fs.existsSync('client/src/routes/RootRouter.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'routes', 'RootRouter.js'), 'client/src/routes/RootRouter.js') }
    if (!fs.existsSync('client/src/routes/UsersRouter.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'client', 'src', 'routes', 'UsersRouter.js'), 'client/src/routes/UsersRouter.js') }
  }
  ...
  add_server_dir() {
    ...
    if (!fs.existsSync('server/app.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'server', 'app.js'), 'server/app.js') }
  }
  ...
  add_test_dir() {
    ...
    if (!fs.existsSync('test/features/title_test')) { fs.copyFileSync(path.join(__dirname, 'files', 'express-react', 'test', 'features', 'title_test.js'), 'test/features/title_test.js') }
...
module.exports = InitExpressReact
