...
class InitPostgresqlSimple {
  ...
  add_readme(ctx) {
    ...
    text = text.replace('__DESCRIPTION__', 'A sandbox for PostgreSQL code.')
  ...
  add_package_json(ctx) {
    let text = fs.readFileSync(path.join(__dirname, 'files', 'postgresql-simple', 'package.json'), 'utf8')
  ...
  add_db_dir() {
    ...
    if (!fs.existsSync('db/schema.sql')) { fs.copyFileSync(path.join(__dirname, 'files', 'postgresql-simple', 'db', 'schema.sql'), 'db/schema.sql') }
    if (!fs.existsSync('db/migrations/201904220518_create_schema_migrations.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'postgresql-simple', 'db', 'migrations', '201904220518_create_schema_migrations.js'), 'db/migrations/201904220518_create_schema_migrations.js') }
    if (!fs.existsSync('db/seeds/development_seed.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'postgresql-simple', 'db', 'seeds', 'development_seed.js'), 'db/seeds/development_seed.js') }
  }

  add_index_js() {
    if (!fs.existsSync('index.js')) { fs.copyFileSync(path.join(__dirname, 'files', 'postgresql-simple', 'index.js'), 'index.js') }
  }

  npm_install_deps() {
    if (!fs.existsSync('package-lock.json')) {
      child_process.execSync('npm install pg --save --loglevel=error')
...
module.exports = InitPostgresqlSimple
